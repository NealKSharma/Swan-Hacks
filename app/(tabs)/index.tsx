...
const STAT_ROWS: {
  ...
  { key: "Quiet",            label: "quiet spots", iconName: "status-quiet",            iconColor: colors.quiet },
  { key: "Moderate",         label: "moderate",    iconName: "status-moderate",         iconColor: colors.moderate },
  { key: "Busy",             label: "busy",        iconName: "status-busy",             iconColor: colors.busy },
  { key: "Overstimulating",  label: "louder",      iconName: "status-loud",             iconColor: colors.overstimulating },
];
...
export default function HomeScreen() {
  ...
  return (
    <View style={styles.canvas}>
      {/* Decorative blobs peeking from corners — cardinal + gold on white */}
      <View style={[styles.blobGoldLg, { top: insets.top - 60 }]} />
      <View style={styles.blobGoldSm} />
      <View style={styles.blobCardinal} />
        ...
        <View style={styles.statList}>
          {STAT_ROWS.map((row) => (
            <View key={row.key} style={styles.statRow}>
              ...
              <Icon name={row.iconName} size={22} color={row.iconColor} />
            </View>
          ))}
...
const styles = StyleSheet.create({
  ...
  // Hero
  heroBlock: { marginTop: spacing.xl, gap: 0 },
  heroLine: {
    fontSize: 44,
    fontWeight: "700",
    lineHeight: 50,
